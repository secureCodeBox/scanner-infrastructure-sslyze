...
class SSLyzeScan extends EventEmitter {
  ...
  initChild() {
    this.startTimer();
    console.log("Initializing child: " + sslyze.pythonLocation + " with arguments " + this.command);
    ...
    this.child.on('error', (err) => {
      this.killChild();
      if (err.code === 'ENOENT') {
        this.emit('error', 'Python not found at command location: ' + sslyze.pythonLocation)
      } else {
